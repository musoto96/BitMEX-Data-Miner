...
mongoose.connect(db_uri).then(() => {
  const pagingInterval = setInterval(() => {
    if (page < count) {
      extract(testnet=false, method='GET', endpoint='trade', symbol='XBTUSD', data={count: bsize, start: page*bsize, reverse: true})
        .then((data) => {
          console.log(`Data collected: ${data.length}`);
          data.forEach((trade) => {
            const new_trade = new Trade(trade);
            mongoose.model("Trade").find({ "trdMatchID": new_trade.trdMatchID }, (err, matches) => {
              if (err) {
                console.log(err);
              } else {
                if (matches.length === 0) {
